...
class Database:
    ...
    async def _init_tables(self) -> None:
        ...
        # 添加新列（如果不存在）
        try:
            await self._db.execute('ALTER TABLE videos ADD COLUMN max_quality INTEGER')
        except:
            pass
        try:
            await self._db.execute('ALTER TABLE videos ADD COLUMN upload_failed INTEGER DEFAULT 0')
        except:
            pass
        try:
            await self._db.execute('ALTER TABLE videos ADD COLUMN fav_id INTEGER')
        except:
            pass
        try:
            await self._db.execute('ALTER TABLE videos ADD COLUMN fav_title TEXT')
        except:
            pass
        try:
            await self._db.execute('ALTER TABLE videos ADD COLUMN fav_time INTEGER')
        except:
            pass
